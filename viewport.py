...
class Viewport(QFrame):
    ...
    def __init__(self, parent, id, view_dir_, num_vols, coords_outside_, zoom_method_, pan_method_,
        ...
        self.image_view.getView().scene().sigMouseClicked.connect(self.mouse_press_event)
        self.image_view.getView().scene().sigMouseMoved.connect(self.mouse_drag_event)
    ...
    def toggle_drag_mode(self):
        """Toggle between panning and painting modes."""
        if self.is_painting:
            # Disable panning (mouse control in ViewBox)
    ...
    def mouse_press_event(self, event):
        """Handle mouse press events for painting."""
        if self.is_painting:
            # Custom behavior for painting
            coords = self.image_view.getImageItem().mapFromScene(event.scenePos())
            cursor_plot_col = int(coords.x())
            cursor_plot_row = int(coords.y())

            print(f'Painting at col: {cursor_plot_col}, row: {cursor_plot_row}')
            ...
            if voxel_ijk is not None:
                # Update the voxel value at the clicked position
                self.update_voxel_value(voxel_ijk, 1000)

    def mouse_drag_event(self, event):
        """Handle mouse drag events for painting."""
        if self.is_painting:
            coords = self.image_view.getImageItem().mapFromScene(event)
            cursor_plot_col = int(coords.x())
            cursor_plot_row = int(coords.y())

            print(f'Dragging and painting at col: {cursor_plot_col}, row: {cursor_plot_row}')

            voxel_ijk = self.volume_stack[0].screenxy_to_imageijk(self.view_dir, cursor_plot_col,
                                                                  cursor_plot_row,
                                                                  self.current_display_index)
            if voxel_ijk is not None:
                # Update the voxel value during dragging
                self.update_voxel_value(voxel_ijk, 1000)
